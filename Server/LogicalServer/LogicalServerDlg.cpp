...
void CLogicalServerDlg::OnTimer(UINT_PTR nIDEvent)
{
	...
	{
		int tcpmc = LogicalMain::GetInstance()->GetTcpMaxClients();
		int wsmc = LogicalMain::GetInstance()->GetWsMaxClients();
		int tcpcc = TcpSvcManage::GetInstance()->GetConnectedCount();
		int wsgocc = WsGoSvcManage::GetInstance()->GetConnectedCount();
		CString connectedStr;
		connectedStr.Format(L"%d/%d", tcpcc + wsgocc, tcpmc + wsmc);
		((CEdit *)GetDlgItem(IDC_EDIT6))->SetWindowText(connectedStr);
		connectedStr.Format(L"%d/%d", tcpcc, tcpmc);
		((CEdit *)GetDlgItem(IDC_EDIT8))->SetWindowText(connectedStr);
		connectedStr.Format(L"%d/%d", wsgocc, wsmc);
