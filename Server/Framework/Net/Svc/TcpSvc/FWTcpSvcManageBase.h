...
public:
	...
	*/
	bool Start(unsigned short port,
		...
		FWTcpListenSocket::AddrFamily addrFamily = FWTcpListenSocket::AF_V4ONLY);
...
protected:
	...
	*/
	...
	return > 0：buf里的数据能够组成一个或多个完整消息包（如果能组成多个完整消息包，就必须返回这多个包的大小之和，因为此回调函数的设计为边缘触发），稍后将会调用_onTcpSvcRecvCompleted
	return < 0：断开此连接
	*/
