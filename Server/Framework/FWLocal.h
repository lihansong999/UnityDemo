...
#define FW_DEPRECATED	__declspec(deprecated)
...
#define FW_LOG_INTACT(format, ...) \
	do{ \
		Framework::FWOutPut::GetInstance()->Log(Framework::FWOutPut::INTACT, NULL, 0, format, ##__VA_ARGS__); \
			}while(false)

#define FW_LOG(format, ...) \
	do{ \
		Framework::FWOutPut::GetInstance()->Log(Framework::FWOutPut::INFO, __FUNCTION__, __LINE__, format, ##__VA_ARGS__); \
			}while(false)

#define FW_LOG_WRNING(format, ...) \
	do{ \
		Framework::FWOutPut::GetInstance()->Log(Framework::FWOutPut::WARNING, __FUNCTION__, __LINE__, format, ##__VA_ARGS__); \
			}while(false)

#define FW_LOG_ERROR(format, ...) \
	do{ \
		Framework::FWOutPut::GetInstance()->Log(Framework::FWOutPut::WRONG, __FUNCTION__, __LINE__, format, ##__VA_ARGS__); \
			}while(false)
...
#define FW_ASSERT2(cond, msg)	assert(cond)
#define FW_ASSERT(cond)			FW_ASSERT2(cond,"")
...
#define FW_ASSERT2(cond, msg)	do {} while (false)
#define FW_ASSERT(cond)	do {} while (false)
