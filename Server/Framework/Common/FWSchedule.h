...
public:
	FWSchedule() :_running(false), _runMsInvl(0), _scheduleIDBase(0), _iteration(Framework::FW_TIME2()) {};
	...
	*/
	FW_ID Add(T func, long long msInvl, unsigned int count = UINT_MAX, bool hpFirst = false, bool hpCount = false, bool pause = false)
	{
		...
		psd->_count = count;
		psd->_msInvl = msInvl;
		psd->_iteration = (psd->_hpFirst ? Framework::FW_TIME2() : _iteration);
		psd->_func = func;
	...
	*/
	void Resume(FW_ID id)
	{
		...
		if (it != _finalMap.end())
		{
			it->second->_iteration = (it->second->_hpFirst ? Framework::FW_TIME2() : _iteration);
		...
		else
		{
			...
			if (it2 != _addMap.end())
			{
				it2->second->_iteration = (it2->second->_hpFirst ? Framework::FW_TIME2() : _iteration);
